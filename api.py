...
Отчёты сотрудников создаются через PWA (POST /api/report/submit) и пишутся
в Google Sheets. (Ранее часть отчётов писал Telegram-бот напрямую — он выведен
из эксплуатации; старые строки без ID обрабатываются позиционным фолбэком.)
"""
